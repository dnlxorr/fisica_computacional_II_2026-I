...
def energia_cinetica(phi_k, k_arr):
    """
    Energía cinética media ⟨E⟩ con grilla completa de k.
    ⟨E⟩ = Σ_k (ℏk)²/2m |φ_k|² / N
    """
    ek = hbar**2 * k_arr**2 / (2 * m)
    return np.sum(ek * np.abs(phi_k)**2) / N
...
normas     = [norma(psi, x)]
energias   = [energia_cinetica(phi_k, k_arr)]
...
for step in range(1, n_steps + 1):
    ...
    energias.append(energia_cinetica(phi_k, k_arr))
