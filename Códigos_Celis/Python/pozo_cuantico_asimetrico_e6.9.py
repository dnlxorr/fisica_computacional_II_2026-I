...
def autovectores_autovalores(A):
    ...
    n_max = 5000
    N = len(A)
    V = np.eye(N)   # acumula el producto de todas las Q
    ...
    # ordenar eigenvalores de menor a mayor y reordenar
    # los eigenvectores en el mismo orden.
    # El algoritmo QR produce eigenvalores en orden arbitrario.
    # Sin este paso, evals[0] no es el estado base sino cualquier estado,
    # y la comparación entre 10x10 y 100x100 de la parte d) compara
    # estados que no se corresponden.
...
def construccion_H(size):
    """Construye la matriz hamiltoniana de dimensión size x size."""
    H = np.zeros((size, size))
    for i in range(size):
        for j in range(size):
            H[i, j] = elem_H_mn(i + 1, j + 1)   # índices 1-based en la fórmula
    return H
...
# ------------------------------------------------------------------
H10 = construccion_H(10)
print(f"Matriz simétrica: {np.allclose(H10, H10.T)}")

evals10, evecs10 = autovectores_autovalores(H10)

print("\n" + "=" * 45)
print("Parte c) H 10×10 — primeros 10 niveles")
print("=" * 45)
...
# ------------------------------------------------------------------
H100 = construccion_H(100)

evals100, evecs100 = autovectores_autovalores(H100)

print("\n" + "=" * 45)
print("Parte d) H 100×100 — primeros 10 niveles")
print("=" * 45)

# ahora que ambos arrays están ordenados, la comparación
# sí corresponde al mismo estado físico en ambas matrices.
for k in range(10):
...
for state in range(3):
    ...
    axes[state].plot(x * 1e10, prob, linewidth=2, color='steelblue')
    axes[state].fill_between(x * 1e10, prob, alpha=0.15, color='steelblue')
